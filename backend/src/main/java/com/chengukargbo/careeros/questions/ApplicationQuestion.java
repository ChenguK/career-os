...
public class ApplicationQuestion { @Id @GeneratedValue(strategy=GenerationType.IDENTITY) Long id;
 ...
 void reconcileObserved(String externalKey,String canonicalKey,String text,AnswerType type,boolean req,Classification c,List<ApplicationQuestionOption> observedOptions,boolean mappingTrusted){boolean changed=!Objects.equals(canonicalQuestionKey,canonicalKey);externalQuestionKey=externalKey;canonicalQuestionKey=canonicalKey;questionText=text;answerType=type;required=req;classification=c;source=Source.ATS;options.clear();options.addAll(observedOptions);if(changed){approvedAnswerLink=null;proposedAnswer=null;approvedAnswer=null;status=mappingTrusted?Status.NEEDS_REVIEW:Status.UNANSWERED;reviewedAt=null;}updatedAt=OffsetDateTime.now();}
 void mappingRevoked(){canonicalQuestionKey=null;approvedAnswerLink=null;proposedAnswer=null;approvedAnswer=null;status=Status.NEEDS_REVIEW;reviewedAt=null;updatedAt=OffsetDateTime.now();}

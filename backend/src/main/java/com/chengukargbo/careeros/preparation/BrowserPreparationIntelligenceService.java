...
public class BrowserPreparationIntelligenceService {
 ...
 private final ApprovedAnswerService answers; private final ApplicantProfileRepository profiles;
 public BrowserPreparationIntelligenceService(ApplicationRepository a,FormObservationSnapshotRepository s,ObservedQuestionRepository o,QuestionResearchService r,ApprovedAnswerService answers,ApplicantProfileRepository p){applications=a;snapshots=s;observed=o;research=r;this.answers=answers;profiles=p;}
 public Response analyze(Long applicationId,JobFamily family,Seniority seniority,String provider){
  ...
  for(ObservedQuestion question:observed.findBySnapshotIdOrderByDisplayOrderAscExternalQuestionIdAsc(snapshot.getId())){
   if(!question.isActive())continue; LikelyQuestion match=byKey.get(normalizeKey(question.getExternalQuestionId())); MatchMethod method=MatchMethod.EXTERNAL_ID; BigDecimal matchConfidence=new BigDecimal("1.00");
   if(match==null){match=byText.get(normalizeText(question.getQuestionText()));method=MatchMethod.REPRESENTATIVE_QUESTION;matchConfidence=new BigDecimal("0.95");}
   if(match==null){method=MatchMethod.NONE;matchConfidence=BigDecimal.ZERO;gaps.add(new Gap("UNMAPPED_OBSERVED_QUESTION",null,question.getQuestionText(),"Review this ATS question and assign a canonical key before preparing an answer",new BigDecimal("1.00")));}
   String key=match==null?null:match.canonicalKey(); if(key!=null)matched.add(key);
   Suggestion suggestion=key==null?null:trusted.get(key); List<Suggestion> suggestions=suggestion==null?List.of():List.of(withConfidence(suggestion,matchConfidence)); boolean missing=suggestion==null;
   if(missing)gaps.add(new Gap("MISSING_TRUSTED_ANSWER",key,question.getQuestionText(),"Provide and explicitly approve an answer before field preparation",match==null?new BigDecimal("1.00"):matchConfidence));
   assessments.add(new ObservedAssessment(question.getId(),question.getExternalQuestionId(),question.getQuestionText(),question.getAnswerType(),question.isRequired(),key,method,matchConfidence,match==null?null:match.probability(),match==null?null:match.confidence(),suggestions,missing));
  }
  List<LikelyQuestion> notObserved=likely.stream().filter(q->!matched.contains(q.canonicalKey())).toList();
  notObserved.forEach(q->gaps.add(new Gap("LIKELY_QUESTION_NOT_OBSERVED",q.canonicalKey(),q.question(),"Prepare this likely question before visiting the application form",q.probability().multiply(q.confidence()).setScale(2,RoundingMode.HALF_UP))));
 ...
 private Suggestion withConfidence(Suggestion s,BigDecimal match){return new Suggestion(s.canonicalKey(),s.source(),s.sourceId(),s.value(),s.confidence().multiply(match).setScale(2,RoundingMode.HALF_UP));}
